...
namespace articleapp.Repo
{
    public class ArticleRepo : IArticles
    {
        ...
        public async Task<List<BasicArticleWithDetails>> GetAllArticlesByUser(string? token = null, int? userId = 14)
        {
        ...
        public async Task<FullArticleWithDetails> GetDetailedArticle(string? token = null, int? articleId = 14)
        {
            string url = $"https://articlesapp.onrender.com/api/Article/GetFullArticleDetails?articleId={articleId}";
            return await HttpRequestHelper.GetRequest<FullArticleWithDetails>(url, temptoken);
        }

        #region saved articles
        public async Task<List<BasicArticleWithDetails>> GetSavedArticlesByUser(string? token = null, int? userId = 14)
        {
